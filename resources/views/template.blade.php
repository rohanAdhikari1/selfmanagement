<div class="fixed inset-0 bg-black/90 touch-pinch-zoom z-[9999]" style="display: none;" role="region" aria-modal="true"
    aria-roledescription="carousel" x-show="$store.lightbox.show[group]" x-transition.opacity.duration.300ms
    x-trap.noscroll="$store.lightbox.show[group]" :aria-hidden="!$store.lightbox.show[group]"
    @touchstart="$store.lightbox.onTouchStart($event)" @touchend="$store.lightbox.onTouchEnd($event, group)"
    @keydown.esc="$store.lightbox.show[group] = null" @keydown.left="$store.lightbox.prev(group)"
    ...
    <ul aria-live="polite" role="presentation" id="lightbox-list">
        <template x-for="(item, index) in $store.lightbox.items[group]" :key="item.id">
            <li class="absolute inset-0 flex justify-center items-center" role="group" aria-roledescription="slide"
                x-show="$store.lightbox.show[group]?.id === item.id" x-transition.opacity.duration.300ms
                ...
                <template x-if="item.type === 'image' && !item.loaded">
                    <div>
                        <svg aria-hidden="true" class="w-9 h-9 text-gray-600 animate-spin fill-white"
                ...
                <template x-if="item.type === 'embed'">
                    <div class="flex items-center w-full self-stretch max-w-[calc(80vh/0.5625)]">
                        <div class="relative w-full h-0 pb-[56.25%]">
                            <iframe class="absolute top-0 left-0 w-full h-full" allowfullscreen
                                :src="item.url"></iframe>
                        </div>
                    </div>
                </template>
                <template x-if="item.type === 'video'">
                    <div class="flex items-center w-full self-stretch max-w-[calc(80vh/0.5625)]">
                        <div class="relative w-full h-0 pb-[56.25%]">
                            <video class="absolute top-0 left-0 w-full h-full" :src="item.url" controls
                                playsinline x-data="{
                                    setAttr(attr, val) {
                                        $el[attr] = val
                                    }
                                }" x-init="setAttr('autoplay', item.autoplay)
                                setAttr('muted', item.muted)
                                $watch('item.autoplay', (val) => setAttr('autoplay', val))
                                $watch('item.muted', (val) => setAttr('muted', val))"></video>
                        </div>
                    </div>
                </template>
            </li>
    ...
    <a href="#" class="absolute top-1/2 left-4 -translate-y-1/2 bg-black/40 p-3 md:p-4 rounded" role="button"
        aria-controls="lightbox-list" aria-label="Previous" x-show="$store.lightbox.items[group].length > 1"
        @click.prevent="$store.lightbox.prev(group)">
        <svg class="w-4 h-4 md:w-5 md:h-5 fill-white" viewBox="0 0 382 382" version="1.1"
    ...
    <a href="#" class="absolute top-1/2 right-4 -translate-y-1/2 bg-black/40 p-3 md:p-4 rounded" role="button"
        aria-controls="lightbox-list" aria-label="Next" x-show="$store.lightbox.items[group].length > 1"
        ...
        <svg class="w-4 h-4 md:w-5 md:h-5 fill-white" viewBox="0 0 382 382" version="1.1"
            ...
            <path
                ...
                transform="translate(201.685326, 191.000000) scale(-1, 1) translate(-201.685326, -191.000000)"></path>
        </svg>
    </a>
    <a href="#" class="absolute top-4 right-4 bg-black/40 p-3 md:p-4 rounded" role="button"
        aria-controls="lightbox-list" aria-label="Close" @click.prevent="$store.lightbox.show[group] = null">
        <svg class="w-4 h-4 md:w-5 md:h-5 fill-white" viewBox="0 0 382 382" version="1.1"
            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            <path
